...
Plačiau: [Aplinkos paruošimas → Reikalingų agentų skaičius](agento-paruošimas.md#reikalingu-agentu-skaicius)

### 2. Kaip diegti TEST agentą?

TEST agentą galima įdiegti dviem būdais:

- **Atskira VM** — rekomenduojamas variantas; pilna izoliacija, tačiau reikia papildomos VM ir tinklo derinimo
- **Docker konteineris PROD VM** — greitesnis būdas; nereikia naujos VM, tačiau TEST ir PROD dalinasi resursais ir yra priklausomi vienas nuo kito

Plačiau: [Aplinkos paruošimas → TEST agento infrastruktūros variantai](agento-paruošimas.md#test-agento-infrastrukturos-variantai)

### 3. Kokie vartai bus naudojami?

Nuo vartų tipo priklauso tinklo konfigūravimo apimtis — ši dalis trunka ilgiausiai, todėl ją reikia **užsisakyti kuo anksčiau**:

- **Vidiniai vartai** — institucija turi būti KVTC klientė ir įtraukta į SVDPT naudotojų sąrašą; reikalingas papildomas tinklo sujungimas per SVDPT (KVTC forma → VSSA)
- **Išoriniai vartai** — papildomos tinklo konfigūracijos nereikia; tinkamas institucijoms, kurių IS nėra SVDPT tinkle
