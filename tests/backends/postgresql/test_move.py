...
def test_move_patch_simple(
    ...
    resp = app.post('/datasets/redirect/patch/Country', json={
        '_id': lt_id,
        'id': 0,
        'name': 'Lithuania'
    })
    ...
    resp = app.get(f'/datasets/redirect/patch/Country')
    ...
    resp = app.patch(f'/datasets/redirect/patch/Country/{lt_id}/:move', json={
        '_id': new_lt_id,
        '_revision': revision
    })
    ...
    redirect_data = list(_redirect_table_data(context, 'datasets/redirect/patch/Country'))
    assert redirect_data == [{
        '_id': lt_id,
        'redirect': new_lt_id
    }]

    resp = app.get(f'/datasets/redirect/patch/Country/:changes')
    assert resp.status_code == 200
...
def test_move_delete_referenced(
    ...
    app = create_test_client(context)
    app.authorize(
        ['spinta_insert', 'spinta_getall', 'spinta_wipe', 'spinta_changes', 'spinta_set_meta_fields', 'spinta_move', 'spinta_patch'])
    lt_id = "af697924-00ef-4dda-b212-0666f74b5365"
