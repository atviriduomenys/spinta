...
def test_migrate_long_names(
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        _clean_up_tables(meta, ['migrate/example/very/very/long/dataset/name/ExtremelyLongModelName',
...
def test_migrate_string_to_text(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/Test"]
        columns = table.columns
        assert {'text', '__text_lt', '__text_en'}.issubset(
            columns.keys())
        assert not {'__text', 'text_lt', 'text_en'}.issubset(
            columns.keys())
...
def test_migrate_string_to_text_rename(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/Test"]
        columns = table.columns
        assert {'text', '__text_lt'}.issubset(
            columns.keys())
        assert not {'__text', 'text_lt'}.issubset(
            columns.keys())
...
def test_migrate_string_to_text_advanced_with_rename(
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/Test"]
        columns = table.columns
        assert {'other', '__text_lt'}.issubset(
            columns.keys())
        assert not {'__text', 'text_lt'}.issubset(
            columns.keys())
