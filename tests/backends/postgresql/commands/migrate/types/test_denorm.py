...
def test_migrate_do_nothing_ref_3_denorm(
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/City"]
        columns = table.columns
        assert {'id', 'country.id', 'country.name'}.issubset(
            columns.keys())
        assert not {'__country.id'}.issubset(
            columns.keys())
...
def test_migrate_adjust_ref_levels_with_denorm(
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables['migrate/example/Ref']
        columns = table.columns
        assert {'someText', '__someRef._id', 'someRef.someText', 'someRef.someNumber', 'someRef.id',
                'someRef.text'}.issubset(columns.keys())
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables['migrate/example/Ref']
        columns = table.columns
        assert {'someText', '__someRef._id', 'someRef._id', '__someRef.someText', '__someRef.someNumber', 'someRef.id',
                'someRef.text'}.issubset(
            columns.keys())
...
def test_migrate_ref_4_add_denorm(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
...
def test_migrate_ref_4_remove_denorm(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/City"]
        columns = table.columns
        assert {'id', 'country._id', '__country.name'}.issubset(
            columns.keys())
        assert not {'__country._id', 'country.name'}.issubset(
            columns.keys())
...
def test_migrate_ref_4_rename_denorm(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/City"]
        columns = table.columns
        assert {'id', 'country._id', 'country.test'}.issubset(
            columns.keys())
        assert not {'__country._id', 'country.name'}.issubset(
            columns.keys())
...
def test_migrate_ref_nesting_do_nothing(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/City"]
        columns = table.columns
        assert {'id', 'country._id', 'country.ctr._id', 'country.ctr.id'}.issubset(
            columns.keys())
        assert not {'__country._id', '__country.ctr._id', '__country.ctr.id'}.issubset(
            columns.keys())
...
def test_migrate_ref_nested_update(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
...
def test_migrate_ref_nested_ref_to_scalar(
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/City"]
        columns = table.columns
        assert {'id', 'country._id', 'country.ctr', '__country.ctr._id'}.issubset(
            columns.keys())
        assert not {'__country._id', 'country.ctr._id'}.issubset(
            columns.keys())
...
def test_migrate_ref_nested_text(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
...
def test_migrate_ref_3_add_denorm(
    postgresql_migration: URL,
    rc: RawConfig,
    cli: SpintaCliRunner,
    tmp_path: Path
...
def test_migrate_object(
    ...
    with sa.create_engine(postgresql_migration).connect() as conn:
        ...
        table = tables["migrate/example/City"]
        columns = table.columns
        assert {'id', 'country.test'}.issubset(
            columns.keys())
        assert not {'country.name'}.issubset(
            columns.keys())
