...
def test_basic_structure():
    ...
    assert set(open_api_spec.keys()) == set(['openapi', 'info', 'servers', 'tags', 'externalDocs', 'paths', 'components', 'x-tagGroups'])
...
def test_components_schemas():
    ...
    all_model_schemas = [
        f"{model}{schema}" for model in expected_models 
...
def test_components_paths():
    ...
    all_model_paths = [
        f"/{dataset_name}/{model}{path_type}" 
    ...
    assert expected_paths.issubset(actual_paths), \
        f"Missing paths: {expected_paths - actual_paths}"
...
def _validate_operation_id_contains(operation_id, path, *required_terms):
    ...
    for term in required_terms:
        assert term in operation_id, \
            f"OperationId '{operation_id}' should contain '{term}' for {path}"
...
def _validate_response_schema(responses, path, expected_ref):
    ...
    assert schema["$ref"] == expected_ref, \
        f"Schema ref should be '{expected_ref}', got '{schema['$ref']}' for {path}"
...
def test_only_head_and_get_operations():
    ...
    for path, operations in paths.items():
        ...
        http_methods = {
            method.lower() for method in actual_methods 
...
def _test_base_model_schema(schemas, model_name, expected_properties):
    ...
    for prop_name in expected_properties:
        ...
        assert "type" in prop_schema or "$ref" in prop_schema, \
            f"Property {prop_name} missing type/ref in {model_name}"
