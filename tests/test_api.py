...
def test_auth_clients_create_authorized_correct(
    ...
    assert resp.status_code == 200
    assert resp.json() == {"client_id": resp_json["client_id"], "client_name": resp_client_name, "scopes": resp_scopes}
    if not client_name:
...
def test_auth_clients_update_authorized_correct(rc: RawConfig, tmp_path: pathlib.Path):
    ...
    assert resp.status_code == 200
    assert resp.json() == {"client_id": resp_json["client_id"], "client_name": "TEST", "scopes": ["spinta_getall"]}
    client = query_client(path, client=resp_json["client_id"])
