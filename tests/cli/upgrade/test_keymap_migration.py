...
def test_upgrade_missing_redirect_migration_entry(
    ...
    with keymap:
        ...
        keymap.conn.execute(migration_table.delete().where(
            migration_table.c.migration == Script.SQL_KEYMAP_REDIRECT.value
        ))
...
def test_upgrade_redirect_migration_from_old_version_with_data(
    ...
    with keymap:
        ...
        first_entry_id = keymap.encode(
            name="syncdataset/countries/Country",
            value=1
        )
        second_entry_id = keymap.encode(
            name="syncdataset/countries/Country",
            value=5
        )
        third_entry_id = keymap.encode(
            name="syncdataset/countries/Country",
            value=10
        )
...
def test_upgrade_redirect_migration_from_old_version_with_multi_column_data(
    ...
    with keymap:
        ...
        first_entry_id = keymap.encode(
            name=city_model,
            value=[1, "Vilnius", country_id_1]
        )
        second_entry_id = keymap.encode(
            name=city_model,
            value=[5, "Kaunas", country_id_1]
        )
        third_entry_id = keymap.encode(
            name=city_model,
            value=[10, "Siauliai", country_id_1]
        )
