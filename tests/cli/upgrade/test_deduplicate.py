...
def test_upgrade_deduplicate_simple(
    ...
    assert listdata(result, 'id', 'name', 'country', '_revision', '_id', full=True) == data[
        'datasets/deduplicate/cli/City']
    ...
    result = app.get('datasets/deduplicate/cli/City')
    assert listdata(result, 'id', 'name', 'country', '_id', '_revision', full=True) == data[
        'datasets/deduplicate/cli/City']
...
def test_upgrade_deduplicate_referenced(
    ...
    data = {
        'datasets/deduplicate/cli/Country': [{
            ...
            '_id': '2582807c-f4d5-4026-8fa8-023088d747bb',
            'id': 1,
            'name': 'Poland'
        }],
        'datasets/deduplicate/cli/City': [{
    ...
    assert not any(
        constraint["name"] in (
            uq_country_constraint,
            uq_city_constraint,
            uq_random_constraint
    ...
    result = app.get('datasets/deduplicate/cli/City')
    assert listdata(result, 'id', 'name', '_id', 'country', '_revision', full=True) == data[
        'datasets/deduplicate/cli/City']
    ...
    resp = app.get(f'/datasets/deduplicate/cli/Country/:changes/-1')
    assert resp.status_code == 200
