...
def test_base_select(rc, postgresql, request, scopes: list):
    ...
    _id = str(uuid.uuid4())
    app.post("/datasets/gov/example/base/Location", json={"_id": _id, "id": 1, "name": "Base location", "type": "city"})
    app.post("/datasets/gov/example/base/City", json={"_id": _id, "name": "City", "population": 100})

    resp = app.get("/datasets/gov/example/base/City?select(id,name,_base.name,population,_base.type)")
    assert resp.json()["_data"] == [
        {"_base": {"name": "Base location", "type": "city"}, "id": 1, "name": "City", "population": 100}
    ]
