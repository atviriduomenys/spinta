    ...
    with create_sqlite_db(
        {
            "language": [
                sa.Column("name", sa.Text),
                sa.Column("id", sa.Integer),
                sa.Column("_id", sa.String, default=lambda: str(uuid.uuid4())),
            ],
        }
    ) as db:
        db.write(
            "language",
            [
                {"id": 0, "_id": "1590ab44-6463-4da7-8862-3598f6e83924", "name": "English"},
                {"id": 1, "_id": "91119bc3-905a-4eff-a2d0-7d9c46711cfa", "name": "Lithuanian"},
                {"id": 2, "_id": "780c9eb7-30c6-4c00-8d11-4c5aec272a54", "name": "Polish"},
            ],
        )
...
@pytest.fixture(scope="module")
def _id_string_column():
    with create_sqlite_db(
        {
            "language": [
                sa.Column("name", sa.Text),
                sa.Column("id", sa.Integer),
                sa.Column("_id", sa.String),
            ],
        }
    ) as db:
        db.write(
            "language",
            [
                {"id": 0, "_id": "stringas1", "name": "English"},
                {"id": 1, "_id": "stringas2", "name": "Lithuanian"},
                {"id": 2, "_id": "stringas3", "name": "Polish"},
            ],
        )
...
def test_id_uuid(context, rc, tmp_path, _id_uuid_column):
    ...
    app = create_client(rc, tmp_path, _id_uuid_column, mode="external")
    resp = app.get("/example/Language")
    lang_data = resp.json()["_data"]
    assert lang_data == [
        {
            "_type": "example/Language",
        ...
        {
            "_type": "example/Language",
            "_id": "91119bc3-905a-4eff-a2d0-7d9c46711cfa",
            "_revision": None,
            "name": "Lithuanian",
            "id": "1",
        },
        {
            "_type": "example/Language",
            "_id": "780c9eb7-30c6-4c00-8d11-4c5aec272a54",
            "_revision": None,
            "name": "Polish",
            "id": "2",
        },
    ]
    getone = app.get("/example/Language/1590ab44-6463-4da7-8862-3598f6e83924")
    assert getone.status_code == 200, getone.json()
    assert getone.json() == {
        "_type": "example/Language",
        "_id": "1590ab44-6463-4da7-8862-3598f6e83924",
        "name": "English",
