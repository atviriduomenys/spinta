...
def ufunc(context, engine):
    ...
    op = Operations(ctx)
    return UFuncTester(Alembic, context, scope={"op": op, "backend": create_empty_backend(context, "postgresql")})
...
def test_drop_column(engine, ufunc, request):
    meta = sa.MetaData(engine)
    table = sa.Table("_test_drop_column_table", meta, sa.Column("id", sa.String()), sa.Column("name", sa.String()))
    table.create()
