...
def test_scalar_with_empty_saved(context):
    ...
    patch = {"scalar": "42"}
    saved = {}
    full_patch = build_full_response(context, model, patch=patch, saved=saved)
    assert full_patch == {
        "scalar": "42",
    }
...
def test_obj_with_default(context):
    ...
    saved = {}
    full_patch = build_full_response(context, model, patch=patch, saved=saved)
    assert full_patch == {"obj": {"foo": "def_val"}}
...
def test_obj_with_default_and_non_empty_saved(context):
    ...
    assert full_patch == {"obj": {"foo": "non_default"}}
...
def test_nested_obj(context):
    ...
    saved = {"obj": {"foo": "13"}}
    full_patch = build_full_response(context, model, patch=patch, saved=saved)
    assert full_patch == {"obj": {"foo": None, "sub": {"foos": "420"}}}
