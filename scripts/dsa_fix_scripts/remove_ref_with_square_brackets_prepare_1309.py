...
def process_csv_file(file_path):
    ...
    for i, row in enumerate(rows):
        ...
        prepare_value, brackets_removed = remove_empty_bracket_words(original_prepare)

        # If no change was made or no brackets were removed, keep original line
        if prepare_value == original_prepare or not brackets_removed:
            output_lines.append(data_lines[i])
            continue

        changes_made += 1
        print(f"Change #{changes_made} - Removing empty brackets from prepare value")
