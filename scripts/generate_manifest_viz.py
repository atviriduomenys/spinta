...
def make_struktura():
    ...
    rows = [
        ...
        ("② Resource",
         ["", "myapp", "", "", "sql", "myapp_db", "", "", ""],
         C["resource"],
         "resource (stulp. 2) = šaltinio pavadinimas\nref (stulp. 6) = backend iš config.yml"),
        ...
        ("① Dataset",
         ...
         C["dataset"], None),
        ("② Resource",
         ["", "produktai", "", "", "sql", "products_db", "", "", ""],
         C["resource"],
         "kitas backend (products_db) — kita DB,\nresource pavadinimas gali skirtis"),
        ("(tuščia)", [""] * 9, C["separator"], None),
        ("③ Model",
         ["", "", "Product", "", "", "id", "product", "", ""],
         C["model"], None),
