...
def _get_row_value(context: Context, row: Any, sel: Any, params: dict | None) -> Any:
    ...
    if isinstance(sel, Selected):
        if isinstance(sel.prep, Expr):
            ...
            isinstance(sel.prop.dtype, PrimaryKey) 
            and isinstance(sel.prep, Selected)
            and isinstance(sel.prep.prep, dict)
        ):
            val = {}
...
def dask_get_all(
    ...
    for i, row in qry.iterrows():
        ...
        for key, sel in env.selected.items():
            ...
            if not isinstance(selected_prop_parent_dtype, Text):
                res[key] = val
            elif isinstance(selected_prop_parent_dtype, Text) and isinstance(val, str):
