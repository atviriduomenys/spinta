...
async def get_verification_keys(request: Request) -> JSONResponse:
    ...
    if default_key := load_key_from_file(config, KeyType.public):
        content["keys"].append(default_key)
    return JSONResponse(content=content | {"now": str(datetime.now().isoformat())})
...
def init(context: Context):
    ...
    routes = [
        ...
        Route("/auth/token/verification/keys", get_verification_keys, methods=["GET"]),
