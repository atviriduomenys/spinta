...
from spinta.components import Context, Manifest, Node, Cache, Command, CommandList
...
def load(context: Context, model: Model, data: dict, manifest: Manifest):
    ...
    if isinstance(model.source, list):
        model.source = load(context, CommandList(), [
            _get_source(s, model.parent.source) for s in model.source
        ], argname='source')
    else:
        model.source = load(context, CommandList(), [_get_source(model.source, model.parent.source)], argname='source')
    return model
...
def load(context: Context, prop: Property, data: dict, manifest: Manifest):
    ...
    if isinstance(prop.source, list):
        prop.source = load(context, CommandList(), [
            _get_source(s, prop.parent.source) for s in prop.source
        ], argname='source')
    else:
        prop.source = load(context, Command(), _get_source(prop.source, prop.parent.source), argname='source')
    return prop
...
def pull(context: Context, dataset: Dataset, *, models: list = None):
    with context.enter():
        context.bind('pull.cache', Cache)
        context.bind('pull.session', requests.Session)
...
def _pull(context: Context, model: Model, source, dependency):
    command = load(context, Command(), source, scope='source')
    dataset = model.parent
    rows = command(context, dependency=dependency)
    for row in rows:
        data = {'type': f'{model.name}/:source/{dataset.name}'}
        for prop in model.properties.values():
            if isinstance(prop.source, list):
                data[prop.name] = [
                    _get_value_from_source(prop, command, prop_source, row, dependency)
                    for prop_source in prop.source
                ]

            elif prop.source:
                data[prop.name] = _get_value_from_source(prop, command, prop.source, row, dependency)
    ...
    if deps:
        ...
        if len(command_calls) > 0:
            ...
            for name, cmd in command_calls.items():
                cmd = load(context, Command(), cmd, scope='source')
                for value in cmd(context, model):
                    yield {name: value}
        else:
            model_name = list(model_names)[0]
            params = parse_url_path(model_name)
            for row in getall(params['path'], {'show': prop_names, 'source': params['source']}):
                yield {
...
def _get_value_from_source(context, prop, command, source, value, dependency):
    if prop.dependency:
        return dependency.get(source)
    else:
        return command(context, source, value)
