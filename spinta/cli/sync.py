...
def _get_base_path_and_authorization_headers(context: Context) -> Tuple[str, Dict[str, str]]:
    ...
    organization_path = f"{credentials.organization_form}/{credentials.organization_name}"
    base_path = f"{resource_server}/uapi/datasets/{organization_path}/isris/dcat"
...
def sync(
    ...
    if response_get_dataset.status_code == HTTPStatus.OK:
        ...
        response_update_dsa = requests.put(f"{base_path}/Dataset/{dataset_id}/dsa/")
