...
def keymap_sync(
    ...
    input_source: str = Option(None, "-i", "--input", help=("Input source file")),
    credentials: str = Option(
        None, "--credentials", help=("Credentials file, defaults to {config_path}/credentials.cfg")
    ),
    dataset: str = Option(None, "-d", "--dataset", help=("Sync only specified dataset")),
    dry_run: bool = Option(False, "--dry-run", help=("Run keymap sync in dry-run mode, no changes will be made.")),
    max_error_count: int = Option(
        50, "--max-errors", help=("If errors exceed given number, keymap command will be stopped.")
    ),
    ...
    if not input_source:
        cli_error("Input source is required.")
    ...
    if credentials:
        ...
        if not credsfile.exists():
            cli_error(f"Credentials file {credsfile} does not exist.")
    else:
    ...
    with context:
        ...
        models = commands.traverse_ns_models(
            context, ns, manifest, Action.SEARCH, dataset_=dataset, source_check=not check_all
        )
