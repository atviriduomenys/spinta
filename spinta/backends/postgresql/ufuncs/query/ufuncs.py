...
def select(
    ...
    column = env.backend.get_column(table, dtype.prop, select=True).label(fpr.left.prop.place + "." + dtype.prop.name)
    return Selected(
...
def _denorm_to_foreign_property(env: PgQueryBuilder, dtype: Denorm):
...
def _ne_compare(env: PgQueryBuilder, prop: Property, column, value):
    ...
    subqry1 = (
        ...
        .where(column != None)  # noqa E711, we use != None, because of ORM
        .  # noqa
        alias()
    )
    env.from_ = env.from_.outerjoin(
    ...
    # value.
    return sa.and_(
        subqry1.c._rid != None,  # noqa E711, we use != None, because of ORM
        subqry2.c._rid == None,  # noqa E711, we use == None, because of ORM
    )
...
def checksum(env: PgQueryBuilder, expr: Expr):
    ...
    return ResultProperty(Expr("checksum", *args))
