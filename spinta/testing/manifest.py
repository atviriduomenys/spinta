...
from spinta.manifests.tabular.helpers import normalizes_columns, render_tabular_manifest, striptable
from spinta.testing.context import TestContext, create_test_context
    ...
    if isinstance(manifest, str) and "|" in manifest:
        if manifest_type and manifest_type != "ascii":
            ...
            with open(ascii_file, "w") as f:
                f.write(manifest)
            copy_manifest(context, manifests=[ascii_file], output=output_file, output_type=manifest_type)
        ...
        if manifest_type and manifest_.type != manifest_type:
            ...
            copy_manifest(context, manifests=[manifest], output=output_path, output_type=manifest_type)
