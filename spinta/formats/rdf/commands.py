        ...
        for key, val in manifest_prefixes.items():
            if isinstance(val, UriPrefix):
                if key in prefixes and prefixes[key] != val.uri:
                    raise DuplicateRdfPrefixMissmatch(
                        model.external.dataset, prefix=key, old_value=prefixes[key], new_value=val.uri
                    )
                prefixes[key] = val.uri
...
def _get_attribute_name(name: str, prefix: str, available_prefixes: dict) -> Union[str, QName]:
...
def _create_model_element(model: Model, prefixes: dict, data: dict) -> Element:
    ...
    data, attributes = _get_attributes(model, data, about_name, type_name, revision_name)
    if model_name:
    ...
    return _create_element(name=name, attributes=attributes, children=list(data.values()), nsmap=prefixes)


def _prepare_for_print(model: Model, prefixes: dict, data: dict) -> str:
...
def prepare_data_for_response(
    ...
    *,
    ...
    for prop, val, sel in select_model_props(
        ...
        elem = commands.prepare_dtype_for_response(context, fmt, prop.dtype, val, data=value, action=action, select=sel)
        data[prop.name] = elem
    return data
...
def prepare_dtype_for_response(
    ...
    *,
    ...
    return _create_element(name=data["_elem_name"], text=str(value))
...
def prepare_dtype_for_response(
    ...
    *,
    ...
    return _create_element(name=data["_elem_name"], text=encode_page_values(value).decode("ascii"))
...
def prepare_dtype_for_response(
    ...
    *,
    ...
    return _create_element(name=data["_elem_name"], text=value.isoformat())
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: Ref, value, *, data: Dict[str, Any], action: Action, select: dict = None
):
    ...
    if len(data_dict) == 1 and "_id" in data_dict or "_uri" in data_dict:
        if "_uri" in data_dict:
            ...
            attributes[data["_resource_name"]] = get_model_link(dtype.model, pk=data_dict["_id"].text)
    else:
        ...
        if isinstance(data["_elem_name"], QName):
            ...
            ref_model_elem = _create_element(
                name=ref_model_name, attributes=ref_model_attrs, children=list(data_dict.values())
            )
            children.append(ref_model_elem)
        else:
            data_dict["_type"] = _create_element(name="_type", text=dtype.model.model_type())
    ...
    if attributes or children:
        return _create_element(name=data["_elem_name"], attributes=attributes, children=children)
    else:
        return None
...
def prepare_dtype_for_response(
    ...
    *,
    ...
    if value["_id"] is not None:
        attributes = {data["_about_name"]: get_model_link(dtype.prop.model, pk=data["_id"], prop=dtype.prop.name)}
        return _create_element(name=data["_elem_name"], attributes=attributes)
    else:
        return None
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: Text, value: str, *, data: Dict[str, Any], action: Action, select: dict = None
):
    return _create_element(name=data["_elem_name"], text=str(value))
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: Text, value: dict, *, data: Dict[str, Any], action: Action, select: dict = None
):
    result = []
    for key, item in value.items():
        if item is not None:
            ...
            attributes = None
            if key != "C":
                attributes = {_get_attribute_name("lang", XML, prefixes): key}
            result.append(_create_element(name=data["_elem_name"], attributes=attributes, text=str(item)))
    return result
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: BackRef, value: str, *, data: Dict[str, Any], action: Action, select: dict = None
):
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: JSON, value: object, *, data: Dict[str, Any], action: Action, select: dict = None
):
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: Object, value: dict, *, data: Dict[str, Any], action: Action, select: dict = None
):
...
def prepare_dtype_for_response(
    ...
    *,
    ...
    return _create_element(name=data["_elem_name"], text=base64.b64encode(value).decode("ascii"))
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: Array, value: tuple, *, data: Dict[str, Any], action: Action, select: dict = None
):
...
def prepare_dtype_for_response(
    context: Context, fmt: Rdf, dtype: UUID, value: object, *, data: Dict[str, Any], action: Action, select: dict = None
):
    if value is None or isinstance(value, NotAvailable):
        return None
