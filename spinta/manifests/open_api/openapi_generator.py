    ...
    def convert_to_openapi_schema(self, model_property, all_models: dict = None) -> dict[str, Any]:
        ...
        if self.is_enum_property(model_property):
            ...
            return {
                "type": "string",
                "enum": enum_values,
                "example": enum_values[0] if enum_values else 'UNKNOWN'
            }
        ...
        return self.schema_registry.type_mapping.mappings.get(
            dtype_name, 
            {'type': 'string', 'example': 'Example value'}
        )
...
class PathGenerator:
    ...
    def update_operation_references(self, operations: dict[str, Any], model_name: str, 
                                    path_type: str = "collection", model_property: tuple | None = None) -> dict[str, Any]:
    ...
    def _update_response_schema(self, operation: dict, model_name: str, 
                                path_type: str, model_property: tuple | None):
        ...
        if not (isinstance(response_200, dict) and 'content' in response_200 and 
                'application/json' in response_200['content'] and 
                'schema' in response_200['content']['application/json']):
            return
...
class SchemaGenerator:
    ...
    def create_model_schema(self, model, all_models: dict) -> dict[str, Any]:
        ...
        schema = {
            ...
            "example": self.create_example_object(model)
        }
    ...
    def create_collection_schema(self, model) -> dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "_data": {
                    "type": "array", 
                    "items": {"$ref": f"#/components/schemas/{model.basename}"}
                }
            }
        }
    ...
    def create_changes_schema(self, model) -> dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "_data": {
                    "type": "array", 
                    "items": {"$ref": f"#/components/schemas/{model.basename}Change"}
                }
            }
        }
    ...
    def create_change_schema(self, model) -> dict[str, Any]:
        ...
        properties = {
            ...
            "_txn": {
                "type": "string",
                "example": "792a5029-63c9-4c07-995c-cbc063aaac2c"
            },
            "_created": {
                "type": "string",
                "format": "date-time",
                "example": "2021-07-30T14:03:14.645198"
            },
            "_op": {
                "type": "string",
                "enum": ["insert", "patch", "delete"]
            },
            "_objectType": {
                "type": "string",
                "example": model.basename
            }
        }
...
class TemplateBasedOpenAPIGenerator:
    ...
    def _extract_manifest_data(self, manifest) -> tuple[Any, dict]:
        ...
        rows = _read_and_return_manifest(context, manifests)
    ...
    def _override_paths_section(self, spec: dict[str, Any], datasets: Any, models: dict):
        ...
        for dataset_name, _ in datasets:
            ...
            for model in dataset_models:
                self._replace_model_paths_in_template(
                    original_paths, new_paths, model, dataset_name
                )
    ...
    def _get_dataset_models(self, dataset_name: str, models: dict) -> list:
        ...
        return [
            ...
            if (hasattr(model, 'external') and 
                hasattr(model.external, 'dataset') and
                model.external.dataset.name == dataset_name)
        ]
    
    def _replace_model_paths_in_template(self, original_paths: dict[str, Any], 
                                         new_paths: dict[str, Any], model, dataset_name: str):
