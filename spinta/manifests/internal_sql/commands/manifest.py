...
from spinta.components import Context, Model, Namespace
from spinta.datasets.components import Dataset
from spinta.datasets.inspect.helpers import zipitems
from spinta.exceptions import DatasetNotFound, ModelNotFound, NamespaceNotFound
from spinta.manifests.components import Manifest
from spinta.manifests.internal_sql.components import InternalSQLManifest
from spinta.manifests.internal_sql.helpers import (
    datasets_to_sql,
    get_manifest,
    get_object_from_id,
    get_transaction_connection,
    internal_to_schema,
    load_internal_manifest_nodes,
    load_required_models,
    select_full_table,
    update_schema_with_external,
)
...
def get_model_name_list(
    context: Context, manifest: InternalSQLManifest, loaded: bool, namespace: str = None, recursive: bool = False
):
    ...
    stmt = (
        sa.select(table.c.path)
        .where(
            sa.and_(table.c.path.startswith(namespace), table.c.dim == "model") if namespace else table.c.dim == "model"
        )
...
def get_namespace_name_list(context: Context, manifest: InternalSQLManifest, loaded: bool, namespace: str = None):
    ...
    stmt = (
        sa.select(table.c.mpath)
        .where(
            sa.and_(
                table.c.mpath.startswith(namespace),
                table.c.mpath != namespace,
                sa.or_(table.c.dim == "namespace", table.c.dim == "dataset"),
            )
            if namespace
            else sa.or_(table.c.dim == "namespace", table.c.dim == "dataset")
        )
...
def get_dataset_name_list(context: Context, manifest: InternalSQLManifest, loaded: bool):
    ...
    stmt = sa.select(table.c.path).where(table.c.dim == "dataset").order_by(table.c.path)
    rows = conn.execute(stmt)
    for row in rows:
        yield row["path"]
...
def has_model(context: Context, manifest: InternalSQLManifest, model: str, loaded: bool = False, **kwargs):
    ...
    if not loaded and conn is not None:
        table = manifest.table
        return conn.execute(
            sa.select([sa.exists().where(sa.and_(table.c.path == model, table.c.dim == "model"))])
        ).scalar()
    return False
...
def get_model(context: Context, manifest: InternalSQLManifest, model: str, **kwargs):
    ...
    if conn is not None:
        ...
        model_obj = conn.execute(
            select_full_table(table).where(sa.and_(table.c.path == model, table.c.dim == "model"))
        ).one()
        props = conn.execute(
            select_full_table(table)
            .where(sa.and_(table.c.path.startswith(f"{model}/"), table.c.dim != "model"))
            .order_by(table.c.index)
        )
        ...
        schemas = update_schema_with_external(schemas, {"dataset": parent_dataset, "resource": parent_resource})
...
def has_namespace(context: Context, manifest: InternalSQLManifest, namespace: str, loaded: bool = False, **kwargs):
    ...
    if conn is not None and not loaded:
        ...
        return conn.execute(
            sa.select(
                [sa.exists().where(sa.or_(table.c.mpath == namespace, table.c.mpath.startswith(f"{namespace}/")))]
            )
        ).scalar()
    return False
...
def get_namespace(context: Context, manifest: InternalSQLManifest, namespace: str, **kwargs):
    ...
    if conn is not None:
        ...
        ns = conn.execute(
            select_full_table(table)
            .where(sa.and_(table.c.name == namespace, table.c.dim == "ns"))
            .order_by(table.c.index)
        )
...
def has_dataset(context: Context, manifest: InternalSQLManifest, dataset: str, loaded: bool = False, **kwargs):
    ...
    if conn is not None and not loaded:
        table = manifest.table
        return conn.execute(
            sa.select([sa.exists().where(sa.and_(table.c.mpath == dataset, table.c.dim == "dataset"))])
        ).scalar()
    return False
    ...
    if conn is not None:
        ...
        return conn.execute(
            sa.select(
                [
                    sa.exists().where(
                        sa.and_(table.c.path == dataset.name, table.c.dim == "resource", table.c.name == resource)
                    )
                ]
            )
        ).scalar()
        ...
        resources = conn.execute(
            ...
            .where(
                sa.and_(
                    ...
                    sa.or_(sa.and_(table.c.name == resource, table.c.dim == "resource"), table.c.dim == "dataset"),
                )
            )
            .order_by(table.c.index)
        )
...
def _adjust_indexes(table, conn, from_index, starting_index):
    if from_index is not None:
        rows = conn.execute(select_full_table(table).where(table.c.index > from_index))
        index = starting_index + 1
        for row in rows:
            conn.execute(sa.update(table).where(table.c.id == row["id"]).values(index=index))
            index += 1
...
class RowMergeBuilder:
    ...
    def execute_all(self, table, conn, starting_index=None, last_index=None):
        ...
        for action in self.actions:
            conn.execute(action.query(table))


@commands.update_manifest_dataset_schema.register(Context, InternalSQLManifest, Manifest)
def update_manifest_dataset_schema(
    context: Context, manifest: InternalSQLManifest, target_manifest: Manifest, **kwargs
):
    manifest = get_manifest(context, manifest)
    conn = get_transaction_connection(context)
    ...
    for dataset in datasets.values():
        ...
        zipped_rows = zipitems(target_rows, in_memory_existing_rows, _id_key)
        ...
        for rows in zipped_rows:
            for new, old in rows:
                if old is NA and new is not NA:
                    # Insert action
                    merge_builder.add_action(RowMergeInsertAction(new))
                elif old is not NA and new is not NA:
                    # Update action
                    merge_builder.add_action(RowMergeUpdateAction(new))
                else:
                    # Delete action
                    merge_builder.add_action(RowMergeDeleteAction(old))
                    if old["dim"] == "model":
                        if old["path"] not in models_to_unload:
                            models_to_unload.append(old["path"])

        with manifest.engine.begin() as merge_connection:
            merge_builder.execute_all(table, merge_connection, initial_index, last_index)
