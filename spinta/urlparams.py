...
def _get_expand_expr_from_params(params: UrlParams) -> Expr:
    ast = {
        "name": "expand",
        "args": [arg if isinstance(arg, dict) else {"name": "bind", "args": [arg]} for arg in params.expand],
    }
        ...
        if name == "path":
            ...
            if len(args) == 2:
                ...
                try:
                    ...
                    raise exceptions.InvalidValue(operator="limit", message="Limit must be a number higher than 0")
